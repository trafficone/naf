...
static void skip_ahead(unsigned long long bytes, FILE * IN)
{
    if (IN == stdin)
    {
...
static void read_header(Header * h, FILE * IN)
{
    ...
    h->format_version = fgetc_or_incomplete(IN);
    if (h->format_version < 1 || h->format_version > 2) { die("unknown version (%d) of NAF format\n", h->format_version); }
    ...
    if (h->format_version > 1)
    {
        ...
        switch (t) {
            case 1:
                h->in_seq_type = seq_type_rna;
                //in_seq_type_name = "RNA";
                break;
            case 2:
                h->in_seq_type = seq_type_protein;
                //in_seq_type_name = "protein";
                break;
            case 3:
                h->in_seq_type = seq_type_text;
                //in_seq_type_name = "text";
                break;
            default:
                die("unknown sequence type (%d) found in NAF file\n", t); 
        }
    }

    unsigned char flags = fgetc_or_incomplete(IN);

    h->flags.has_title   = (flags >> 6) & 1;
    h->flags.has_ids     = (flags >> 5) & 1;
    h->flags.has_names   = (flags >> 4) & 1;
    h->flags.has_lengths = (flags >> 3) & 1;
    h->flags.has_mask    = (flags >> 2) & 1;
    h->flags.has_data    = (flags >> 1) & 1;
    h->flags.has_quality =  flags       & 1;



    h->name_separator = fgetc_or_incomplete(IN);
    if (h->name_separator < 0x20 || h->name_separator > 0x7E) { die("unsupported name separator character\n"); }
}


static void skip_title(Header * h, FILE * IN)
{
    if (h->flags.has_title)
    {
        unsigned long long title_size = read_number(IN);
        skip_ahead(title_size, IN);
    }
}


static void skip_ids(Header * h, FILE * IN)
{
    if (h->flags.has_ids)
    {
        read_number(IN);
        unsigned long long compressed_ids_size = read_number(IN);
        skip_ahead(compressed_ids_size, IN);
    }
}


static void skip_names(Header * h, FILE * IN)
{
    if (h->flags.has_names)
    {
        read_number(IN);
        unsigned long long compressed_names_size = read_number(IN);
        skip_ahead(compressed_names_size, IN);
    }
}


static void skip_lengths(Header * h, FILE * IN)
{
    if (h->flags.has_lengths)
    {
        read_number(IN);
        unsigned long long compressed_lengths_size = read_number(IN);
        skip_ahead(compressed_lengths_size, IN);
    }
}


static void skip_mask(Header * h, FILE * IN)
{
    if (h->flags.has_mask)
    {
        read_number(IN);
        unsigned long long compressed_mask_size = read_number(IN);
        skip_ahead(compressed_mask_size, IN);
    }
}


/*static void skip_data(Header * h, FILE * IN)
{
    if (h->flags.has_data)
    {
        read_number(IN);
        unsigned long long compressed_data_size = read_number(IN);
        skip_ahead(compressed_data_size, IN)
    }
}*/
...
static void load_ids(FILE * IN, long long N)
{
